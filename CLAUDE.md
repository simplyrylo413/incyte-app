...
- **IMPORTANT:** [`pm/VISUAL-LANGUAGE-BIBLE.md`](pm/VISUAL-LANGUAGE-BIBLE.md) is the visual source of truth for all UI. INCYTE renders *a believable physical machine, not an app* — industrial hardware realism (matte-black chassis, machined-metal panels, glossy-plastic windows, recessed LCD wells, chrome rivets), never flat/SaaS/fintech/minimalist/flat-cyberpunk styling. Read it before building or restyling any surface. The five materials, the upper-left light source, stacked directional shadows, the locked rim-lit button standard (§14), and the "manufactured not drawn" mandate are non-negotiable.
...
- **IMPORTANT:** Default to running and verifying changes on `http://localhost:3000` (the Next.js dev server). **Never deploy to Netlify unless the user explicitly asks for and authorizes a deploy.** Casual phrases like "ship it", "push the change", or completing a feature do NOT constitute deploy permission — only an explicit instruction like "deploy to netlify" qualifies, and even then state the target site and wait for confirmation before running the deploy command.
