...
| 2026-05-13 | **Equipment popover writes to the entry only**, not the plan template. | When the user changes equipment on a Today card, `entry.equipmentType` is updated; the underlying `plan[i].equipmentType` is left alone. Same-session, same-day change only. If the user wants to update the plan permanently they do it in the Plan view. |
| 2026-05-13 | **Rest timer requires explicit tap to start** — no auto-start on log. | `logSet()` does not call `startRestTimer()`. The user taps the rest pill to start the countdown. Matches the prototype's current behavior; preserves user control over rest pacing. |
| 2026-05-13 | **Destructive actions use native `confirm()` dialogs** for v1. | Port `removeSet`'s and `removeMovement`'s `confirm()` calls as-is. Production has a styled inline confirm pattern in some places but it's inconsistent — defer the unification work. v1 ships with native confirms, which are familiar and unambiguous on iOS. |
| 2026-05-13 | **Footer Save & Exit / Finish Session is reachable from Workout Mode too**, not just Today. | The footer (or its actions) renders inside the Workout Mode view shell so the user can finalize the session without backing out to Today. Exact placement (footer vs eyebrow vs more menu) deferred to Phase 2 HTML pass. |
...
## Resolved questions (decisions log mirror)

All open questions resolved as of 2026-05-13. Original questions preserved below with their answers for context.
...
3. ~~**Equipment popover scope** — does changing equipment on Today rewrite the active entry only, or also propagate to the underlying plan item (`p.equipmentType`)?~~ **DECIDED 2026-05-13 (Albert):** Write to `entry.equipmentType` only. Plan template is left alone; users update the plan permanently in the Plan view.
4. ~~**Rest timer auto-start** — prototype's `logSet` does not auto-start; an explicit "Rest" tap is required. Confirm same in production.~~ **DECIDED 2026-05-13 (Albert):** User must tap to start the rest timer. `logSet()` does not auto-start the countdown. Preserves user control over pacing.
...
6. ~~**`confirm()` dialogs** — prototype uses native `confirm()` (`removeSet`, `removeMovement`). Production uses styled inline confirms inconsistently. Pick a pattern before Phase 4.~~ **DECIDED 2026-05-13 (Albert):** v1 uses native `confirm()` for destructive actions. Unifying production's inconsistent styled-inline pattern is deferred.
7. ~~**Save & exit / Finish session footer placement** — prototype has no equivalent. Should they live in Today (where they are now) or also be reachable from Workout Mode footer?~~ **DECIDED 2026-05-13 (Albert):** Yes — also reachable from Workout Mode. Exact placement (footer vs eyebrow vs more menu) decided in Phase 2 during the HTML pass.
